...
class TIFFSimulatorGUI:
    ...
    def _run_batch_simulation(self):
        ...
        if custom_times_str:
            ...
            batch = BatchSimulator(self.output_dir.get())
            ...
            if preset == "quick":
                batch = PresetBatches.quick_test(self.output_dir.get())
            elif preset == "thesis":
                batch = PresetBatches.masterthesis_full(self.output_dir.get())
            else:
                batch = PresetBatches.publication_quality(self.output_dir.get())
        ...
        # Run Batch
        batch.run(progress_callback=progress_callback)
