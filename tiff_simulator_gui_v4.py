...
class TIFFSimulatorGUI_V4:
    ...
    def _create_batch_tab(self):
        ...
        # Preset Auswahl
        preset_frame = ttk.LabelFrame(self.batch_tab, text="🎯 Batch Preset", padding=10)
        preset_frame.pack(fill=tk.X, padx=10, pady=5)

        ttk.Radiobutton(
            preset_frame,
            text="🚀 Quick Test (3 TIFFs, ~2 Min)",
        ...
        ttk.Radiobutton(
            preset_frame,
            text="🎓 Masterthesis (60+ TIFFs, ~45 Min)",
            variable=self.batch_preset,
            value="thesis"
        ).pack(anchor=tk.W, pady=2)
        ...
        ttk.Radiobutton(
            preset_frame,
            text="📄 Publication Quality (30 TIFFs, ~2 Std)",
            variable=self.batch_preset,
            value="publication"
        ).pack(anchor=tk.W, pady=2)

        # Custom Times
        custom_frame = ttk.LabelFrame(self.batch_tab, text="⚙️ Custom Batch", padding=10)
        ...
        tk.Label(row_rep, text="Wiederholungen:", width=18, anchor=tk.W).pack(side=tk.LEFT)
    ...
    def _run_simulation(self):
        ...
        try:
            ...
            self.root.after(0, lambda: messagebox.showinfo(
                ...
                f"Output: {filepath}\n"
                f"Shape: {tiff_stack.shape}\n"
                f"Engine: V4.0 Optimized"
            ))
